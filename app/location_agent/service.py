...
class LocationAgentService:
    ...
    def _search_precise_address(self, analysis: LocationAnalysis) -> LocationAgentResponse:
        queries = analysis.corrected_queries or ([analysis.raw_address_text] if analysis.raw_address_text else [])
        city_hint = analysis.admin_hints.city_name if analysis.admin_hints is not None else None
        location_detail = enrich_location_detail(analysis.location_detail)

        candidates_by_key: dict[str, LocationCandidate] = {}
        for query in queries:
            for candidate in self.map_searcher.search(query, city_hint):
...
def _candidate_to_resolved_address(
    ...
    *,
    ...
    anchor_address = _normalize_anchor_detail_address(candidate.detail_address or candidate.full_address)
